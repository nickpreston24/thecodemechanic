    ...
    Alpine.directive(
        ...
        (el, {expression}, {effect, cleanup, evaluateLater}) => {
            ...
            if (expression.startsWith("content.")) {
                ...
                expression.startsWith("images.") ||
                expression.startsWith("image.")
            ) {
                key = expression.replace(/^images?\./, "");
                storeName = "images";
            } else if (
                expression.startsWith("videos.") ||
                expression.startsWith("video.")
            ) {
            ...
            const applyValue = (value) => {
                console.log("applyValue() > value :>> ", value);

                const files = Array.isArray(value) ? value : [value];

                console.log("applyValue() > files :>> ", files);


                console.log("applyValue() > tag :>> ", tag);
                if ((tag === "img" || tag === "video") && !files[0]?.trim()) return;

                if (files.length > 1) {
                    if (tag === "img")
                        console.warn(
                            "[x-edit] Multiple images require a container, not <img>.",
                        );
                ...
                if (tag === "img") {
                    if (el.src !== String(files[0])) el.src = files[0];
                    return;
                }
                ...
                const rendered = s.renderMarkdown
                    ? s.renderMarkdown(files[0])
                    : s.render
                        ? s.render(files[0])
                        : files[0];

                if (el.innerHTML.trim() !== String(rendered).trim())
